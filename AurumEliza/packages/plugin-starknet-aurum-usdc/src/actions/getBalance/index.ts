import { Action, IAgentRuntime, ActionExample, elizaLogger, HandlerCallback, State } from '@elizaos/core';
...
export class GetBalanceAction implements Action {
    ...
    async validate(_runtime: IAgentRuntime, message: Message): Promise<boolean> {
        ...
        return addressRegex.test(message.content?.address || message.content?.text || '');
    }
    ...
    async handler(
        ...
        try {
            const address = message.content.address || extractAddress(message.content.text || '');
            elizaLogger.error("🟡 address: ", address);
